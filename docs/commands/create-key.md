...
For public repos, committing `envi.maml` partially defeats the purpose of encryption. Skip `create-key` and let [`envi pack`](/commands/pack) / [`envi unpack`](/commands/unpack) fall back to deriving a key from your manifest file (`package.json`, `Cargo.toml`, etc.) instead — that key is already only available to people with code access, which is the same trust boundary `envi.maml` would have given you.
:::
