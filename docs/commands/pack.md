...
1. **`envi.config.maml` with `encryption_key`** (preferred) - Generated by [`envi create-key`](/commands/create-key) and committed to the repo. The most reliable option because the key is stable across dependency updates.
...
After running [`envi create-key`](/commands/create-key) once and committing `envi.config.maml`:
...
- [`envi create-key`](./create-key) - Generate `envi.config.maml` for a stable shared key that survives manifest changes
