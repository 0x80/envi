...
**`envi.maml` encryption** (committed per-repo key — convenience for private team workflows):
...
- Used as a fallback when `envi.maml` is not present
...
**Custom secret encryption** (when no `envi.maml` or manifest detected, or by choice):
...
# Sender
envi pack
# Output: "Using encryption_key from envi.maml"
# Blob is automatically copied to clipboard - paste into chat/email

# Receiver (any checkout that has envi.maml)
envi unpack
# Output: "Found encryption_key in envi.maml - attempting decryption"
```
...
- Need to commit `envi.maml` once

### Method 2: Automatic Encryption (Manifest-based)

**Best for:** Quick sharing in projects that haven't (yet) set up `envi.maml`, when a manifest file is present (JavaScript/TypeScript, Rust, Go, Python, PHP, etc.)
...
- You haven't set up `envi.maml` yet and want a quick share
