...
**`envi.config.maml` encryption** (committed per-repo key — convenience for private team workflows):

- Generated by [`envi create-key`](/commands/create-key) and committed to the repo
...
- ⚠️ Anyone with read access to the source repo can decrypt — best for **private** repos. On a public repo this gives no confidentiality (and neither does the manifest fallback) — use a custom secret instead

**Automatic encryption** (when a manifest file is detected):

- Used as a fallback when `envi.config.maml` is not present
...
- ⚠️ Blobs become unreadable if the manifest file changes (e.g., after `pnpm install` or any dependency bump)

**Custom secret encryption** (when no `envi.config.maml` or manifest detected, or by choice):
...
### Method 1: `envi.config.maml` Encryption

**Best for:** Private team repos where you'd otherwise hit manifest drift (blobs breaking after `pnpm install`). One-time setup, then sharing is fully automatic.

**One-time setup (any team member):**
...
# Sender
envi pack
# Output: "Using encryption_key from envi.config.maml"
# Blob is automatically copied to clipboard - paste into chat/email

# Receiver (any checkout that has envi.config.maml)
envi unpack
# Output: "Found encryption_key in envi.config.maml - attempting decryption"
```

**Pros:**

- Stable key — survives `pnpm install`, dependency bumps, manifest edits
- Same key drives at-rest encryption for `capture`/`restore`
- No secret management needed
- Works for any project type, regardless of manifest

**Cons:**

- Requires the repo to be **private** (anyone with read access can decrypt)
- Need to commit `envi.config.maml` once

### Method 2: Automatic Encryption (Manifest-based)

**Best for:** Quick sharing in projects that haven't (yet) set up `envi.config.maml`, when a manifest file is present (JavaScript/TypeScript, Rust, Go, Python, PHP, etc.)
...
- Stops working when the manifest file changes (any dependency bump invalidates the key)
...
### Method 3: Custom Secret (Any Project or Long-term Storage)
...
Use **`envi.config.maml` encryption** when:

- The repo is private and you want a stable, no-fuss team workflow
- You also want at-rest encryption for `~/.envi/store/` and the GitHub backup
- Dependency updates regularly invalidate manifest-derived keys for your team

Use **automatic (manifest-based) encryption** when:

- You haven't set up `envi.config.maml` yet and want a quick share
- A manifest file is present and stable for your use case
- Working with supported project types (JS/TS, Rust, Go, Python, PHP, etc.)

Use **custom secret** when:

- The repo is public (so committing a key would defeat the purpose)
- Working with projects without manifest files
- Documenting setup in README or onboarding materials
- Long-term storage needs across different projects
