...
- Each **built-in** ignored directory name (`IGNORED_DIRECTORY_NAMES`) is emitted in **both** a root-anchored (`dist/**`) and a nested (`**/dist/**`) form. fast-glob anchors patterns at `cwd`, so the root-only form would let the glob descend into every per-package `node_modules` and every `apps/*/dist` of a workspace — the dominant cost in a monorepo. (`.gitignore`-derived fallback patterns are anchored separately by `parseGitignoreDirsFallback`, which already emits its own `**/`-prefixed form for non-rooted entries.)
- Registered git worktrees are turned into ignore patterns _before_ globbing (`toWorktreeIgnorePatterns`), so a repo that keeps worktrees under its root doesn't traverse a full checkout per worktree only to discard every match. Their files therefore don't appear in `skippedNestedVcsRoots`; nested working trees git doesn't know about (submodules, nested clones, jj/hg/svn) are still detected after the glob and reported there
- Worktree paths are escaped with `fast-glob`'s `posix.escapePath` before being spliced into a pattern. Unescaped, a worktree directory named `feat*` would produce `.worktrees/feat*/**` and silently swallow the sibling `.worktrees/feat-real` — dropping a real env file rather than merely failing to prune
- `listWorktreePaths()` is read **twice**: once before the glob (to build the ignore patterns) and once after (for `partitionNestedVcsRoots`). The post-glob read is what preserves the mid-teardown guarantee — a worktree registered during the walk is absent from the pre-glob snapshot, and its `.git` pointer is exactly then likely to be missing, so the marker probe can't cover it either. The two lists are unioned
